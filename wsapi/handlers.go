...
func handleMessages(player *models.Player) {
	...
	for {
		...
		if err != nil {
			log.Println("[handleMessages] - player.Conn.ReadMessage error:", err)
			UpdatePlayerDataFromRedis(player)
			handlePlayerDisconnect(player, "player.conn.readmessages error nill in handleMessages")
			break
		}
		UpdatePlayerDataFromRedis(player)
...
func handlePlayerDisconnect(player *models.Player, source string) {
	log.Printf("Player disconnected:%v, reason: %v", player.ID, source)
	playersMutex.Lock()
