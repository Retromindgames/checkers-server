...
type FeatureQuery struct {
	...
	withSerieFeatures *SerieFeatureQuery
...
func (fq *FeatureQuery) QueryGameFeatures() *GameFeatureQuery {
	query := (&GameFeatureClient{config: fq.config}).Query()
...
// QuerySerieFeatures chains the current query on the "serie_features" edge.
func (fq *FeatureQuery) QuerySerieFeatures() *SerieFeatureQuery {
	query := (&SerieFeatureClient{config: fq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := fq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (fq *FeatureQuery) Clone() *FeatureQuery {
	...
	return &FeatureQuery{
		...
		withGameFeatures:  fq.withGameFeatures.Clone(),
		withSerieFeatures: fq.withSerieFeatures.Clone(),
...
func (fq *FeatureQuery) WithGameFeatures(opts ...func(*GameFeatureQuery)) *FeatureQuery {
	query := (&GameFeatureClient{config: fq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	fq.withGameFeatures = query
	return fq
}

// WithSerieFeatures tells the query-builder to eager-load the nodes that are connected to
// the "serie_features" edge. The optional arguments are used to configure the query builder of the edge.
func (fq *FeatureQuery) WithSerieFeatures(opts ...func(*SerieFeatureQuery)) *FeatureQuery {
	query := (&SerieFeatureClient{config: fq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	fq.withSerieFeatures = query
	return fq
}
...
func (fq *FeatureQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Feature, error) {
	var (
		...
		_spec       = fq.querySpec()
		loadedTypes = [2]bool{
			fq.withGameFeatures != nil,
			fq.withSerieFeatures != nil,
		}
	)
	...
	if query := fq.withGameFeatures; query != nil {
		if err := fq.loadGameFeatures(ctx, query, nodes,
			func(n *Feature) { n.Edges.GameFeatures = []*GameFeature{} },
			func(n *Feature, e *GameFeature) { n.Edges.GameFeatures = append(n.Edges.GameFeatures, e) }); err != nil {
			return nil, err
		}
	}
	if query := fq.withSerieFeatures; query != nil {
		if err := fq.loadSerieFeatures(ctx, query, nodes,
			func(n *Feature) { n.Edges.SerieFeatures = []*SerieFeature{} },
			func(n *Feature, e *SerieFeature) { n.Edges.SerieFeatures = append(n.Edges.SerieFeatures, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (fq *FeatureQuery) loadGameFeatures(ctx context.Context, query *GameFeatureQuery, nodes []*Feature, init func(*Feature), assign func(*Feature, *GameFeature)) error {
	...
	query.Where(predicate.GameFeature(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(feature.GameFeaturesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.feature_game_features
		if fk == nil {
			return fmt.Errorf(`foreign-key "feature_game_features" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "feature_game_features" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (fq *FeatureQuery) loadSerieFeatures(ctx context.Context, query *SerieFeatureQuery, nodes []*Feature, init func(*Feature), assign func(*Feature, *SerieFeature)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Feature)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	query.Where(predicate.SerieFeature(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(feature.SerieFeaturesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.feature_serie_features
		if fk == nil {
			return fmt.Errorf(`foreign-key "feature_serie_features" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "feature_serie_features" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
