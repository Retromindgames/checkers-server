...
func (rq *RoundQuery) QueryTransactions() *TransactionQuery {
...
func (rq *RoundQuery) Clone() *RoundQuery {
	...
	return &RoundQuery{
		...
		withTransactions: rq.withTransactions.Clone(),
...
func (rq *RoundQuery) WithTransactions(opts ...func(*TransactionQuery)) *RoundQuery {
	query := (&TransactionClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	rq.withTransactions = query
	return rq
}
...
func (rq *RoundQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Round, error) {
	...
	if query := rq.withTransactions; query != nil {
		if err := rq.loadTransactions(ctx, query, nodes,
			func(n *Round) { n.Edges.Transactions = []*Transaction{} },
			func(n *Round, e *Transaction) { n.Edges.Transactions = append(n.Edges.Transactions, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (rq *RoundQuery) loadTransactions(ctx context.Context, query *TransactionQuery, nodes []*Round, init func(*Round), assign func(*Round, *Transaction)) error {
	...
	query.Where(predicate.Transaction(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(round.TransactionsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.round_transactions
		if fk == nil {
			return fmt.Errorf(`foreign-key "round_transactions" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "round_transactions" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
