...
func (tq *TransactionQuery) QueryRounds() *RoundQuery {
	query := (&RoundClient{config: tq.config}).Query()
...
func (tq *TransactionQuery) Clone() *TransactionQuery {
	...
	return &TransactionQuery{
		...
		withRounds: tq.withRounds.Clone(),
...
func (tq *TransactionQuery) WithRounds(opts ...func(*RoundQuery)) *TransactionQuery {
	query := (&RoundClient{config: tq.config}).Query()
...
func (tq *TransactionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Transaction, error) {
	...
	if query := tq.withRounds; query != nil {
		if err := tq.loadRounds(ctx, query, nodes, nil,
			func(n *Transaction, e *Round) { n.Edges.Rounds = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TransactionQuery) loadRounds(ctx context.Context, query *RoundQuery, nodes []*Transaction, init func(*Transaction), assign func(*Transaction, *Round)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Transaction)
	for i := range nodes {
		if nodes[i].round_transactions == nil {
			continue
		}
		fk := *nodes[i].round_transactions
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "round_transactions" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
