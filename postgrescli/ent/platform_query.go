...
func (pq *PlatformQuery) QueryOperator() *OperatorQuery {
...
func (pq *PlatformQuery) Clone() *PlatformQuery {
	...
	return &PlatformQuery{
		...
		withOperator: pq.withOperator.Clone(),
...
func (pq *PlatformQuery) WithOperator(opts ...func(*OperatorQuery)) *PlatformQuery {
	query := (&OperatorClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pq.withOperator = query
	return pq
}
...
func (pq *PlatformQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Platform, error) {
	...
	if query := pq.withOperator; query != nil {
		if err := pq.loadOperator(ctx, query, nodes,
			func(n *Platform) { n.Edges.Operator = []*Operator{} },
			func(n *Platform, e *Operator) { n.Edges.Operator = append(n.Edges.Operator, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *PlatformQuery) loadOperator(ctx context.Context, query *OperatorQuery, nodes []*Platform, init func(*Platform), assign func(*Platform, *Operator)) error {
	...
	query.withFKs = true
	query.Where(predicate.Operator(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(platform.OperatorColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.platform_operator
		if fk == nil {
			return fmt.Errorf(`foreign-key "platform_operator" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "platform_operator" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
